...
router.post('/', function(req, res) {
    ...
    connection.query(sql1, user_id, user_pw,(err, data)=>{
        try{
            if(data[0].result < 1){ //동일 id가 없다면
                res.send({code:101, errorMessage: err}) //입력하신 ID가 없습니다.
                ...
                data.query(sql2, params, (err, data)=>{
                    if(!err){
                        //res.json({message: '로그인을 성공하였습니다!'})
                        res.send(data[0])
                    }else{ 
                        console.log(err);
                        return res.send({code:102, errorMessage: err});
